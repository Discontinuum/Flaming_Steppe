...
function callbacks.generate_map(params)
	...
	local function clear_tile(x, y, tile)
	...
	local function make_road(x, y)
		if not map:on_board(x,y) then
			return
		end
		if map:get_tile(x,y) == params.terrain_castle or map:get_tile(x,y) == params.terrain_keep or map:get_tile(x,y) == params.terrain_village then
			return
		end
		map:set_tile(x, y, params.terrain_road)
	end
	...
	for road in wml.child_range(params, "road") do
		local windiness = road.windiness or 0
		local calc = function(x, y)
			if x == 0 or x == params.map_width - 1 or y == 0 or y == params.map_height - 1 or map:get_tile(x, y) == params.terrain_wall then
				-- Map borders are impassable
				return math.huge
			end
			local res = 1.0
		...
		wesnoth.log("warn", "road from "..road.from.." to "..road.to)
		local start = special_locs[road.from]
		local finish = special_locs[road.to]
		local path = wesnoth.paths.find_path(
			start[1], start[2], finish[1], finish[2], calc, params.map_width, params.map_height)
		for i, loc in ipairs(path) do
			local locs_set = LS.create()
			build_chamber(loc[1], loc[2], locs_set, 1, 0)
			for x,y in locs_set:stable_iter() do
				wesnoth.log("warn", "road "..tostring(x)..";"..tostring(y))
				make_road(x, y)
			end
		end
	end
